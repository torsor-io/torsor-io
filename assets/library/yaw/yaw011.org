...
Here, we've defined a qubit, ~$alg = qubit()~, and the output is
~Created algebra: <X, Z | herm, unit, anti>~. We'll explain what's going on below.
** Compiler
...
#+END_EXAMPLE
We'll explain what's going on in this expression shortly, but note
that we will largely focus on native ~yaw~ syntax.
...
[1, -1]
#+END_EXAMPLE
A shortcut to finding eigenvalues is to produce the *minimal polynomial*
that $A$ is a root of:
\[
p_\min^{(A)}(A) = \sum_{k=0}^n c_k A^k = 0,
\]
often using relations, e.g. $X^2 - I = 0$ for the Pauli $X$.
